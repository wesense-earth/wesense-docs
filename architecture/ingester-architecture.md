...
> **TODO: `reading_type_name` column and gateway validation**
> 
> **Current state:** Ingesters set `reading_type` (e.g., `pm2_5`, `temperature`) and `unit` (e.g., `µg/m³`, `°C`) but there is no `reading_type_name` column for human-readable display names. Respiro currently maintains a hardcoded label map as a workaround. Additionally, some historical data has missing `unit` fields (e.g., `particles_*` and `*_raw` types) and one ingester wrote `unknown` as a reading type.
> 
> **Principle:** Ingesters are the source of truth for what a sensor measures. The storage gateway defines the standard "boxes" (accepted reading types) and validates incoming data. Respiro reads display metadata directly from the database.
> 
> **Target:**
> 
> 1. **Schema:** Add `reading_type_name` column to `sensor_readings` (same pattern as `data_source` / `data_source_name`)
> 2. **Ingesters:** Set `reading_type`, `reading_type_name`, and `unit` on every reading. The ingester knows the sensor and must provide all three.
> 3. **Gateway:** Define a registry of accepted reading types with expected units. Warn or reject readings with unknown types or missing units. This prevents `unknown` and empty-unit readings from entering the database.
> 4. **Firmware:** Ensure `unit` is set for all reading types including particle bin counts (`count/L`) and raw sensor values.
> 5. **Respiro:** Use `argMax(reading_type_name, timestamp)` to get display names (same query pattern as `data_source_name`). Remove hardcoded `formatReadingType()` labels.
> 
> **Standard reading types (reference):**
> 
...
> 
> This follows the same pattern as the `data_source` / `data_source_name` standardisation (completed 2026-03).
