...
## Core Architecture (Ingesters + Storage)

The core data pipeline uses **specialized ingesters** — each data source type has its own ingester optimized for its specific requirements. Ingesters are thin protocol decoders that publish decoded readings to MQTT and POST to the storage broker. They have no knowledge of P2P networking.
...
graph TB
    ...
    subgraph "Ingesters (wesense-ingester-core)"
        M[Ingester - Meshtastic]
    ...
    S -- "Protobuf" --> EMQX
    S -. "MQTT Discovery" .-> HA
    EMQX -- "wesense/v2/#" --> WSING
    WSING -- "POST /readings" --> GW
    L -- "LoRaWAN" --> TTN
    TTN -- "HTTP Webhook" --> WSING
    M -- "POST /readings" --> GW
...
| --------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| wesense-ingester-meshtastic | `wesense-ingester-meshtastic`                           | Subscribes to mqtt.meshtastic.org (public mode) or local nodes (community mode via `MESHTASTIC_MODE`), decodes, caches positions, geocodes, POSTs to storage broker |
| wesense-ingester-wesense    | `wesense-ingester-wesense`                              | Subscribes to WeSense WiFi + LoRa topics, receives TTN webhooks, decodes protobuf, geocodes, POSTs to storage broker                                                |
| wesense-ingester-core       | `wesense-ingester-core`                                 | Shared library: storage broker client, geocoder, dedup cache, MQTT publisher, Ed25519 signing                                                                       |
| EMQX MQTT Broker            | `wesense` (emqx config)                                 | Routes messages between components, forwards Meshtastic to external services                                                                                        |
| ClickHouse                  | `wesense-clickhouse-live`                               | Time-series database for sensor readings                                                                                                                            |
| OrbitDB + Helia             | `wesense-orbitdb`                                       | Node registry, trust list, attestations (separate from archive storage)                                                                                             |
| Storage Broker + Iroh       | `wesense-storage-broker` + `wesense-archive-replicator` | Receives readings from ingesters, writes to ClickHouse, archives to Iroh, serves Parquet over HTTP                                                                  |

**Why Specialized Ingesters?**

Different data sources have fundamentally different characteristics:

| Data Source      | Position Data                      | Telemetry Data                     | Special Requirements                                             |
| ---------------- | ---------------------------------- | ---------------------------------- | ---------------------------------------------------------------- |
| **Meshtastic**   | Separate POSITION_APP messages     | Separate TELEMETRY_APP messages    | Must cache positions and correlate with telemetry (up to 7 days) |
| **WeSense WiFi** | Included in every protobuf message | Included in every protobuf message | Simple decode + geocode                                          |
| **WeSense LoRa** | Included in every protobuf message | Included in every protobuf message | Simple decode + geocode                                          |

The Meshtastic ingester needs sophisticated caching and correlation logic that would be wasteful for WeSense sensors. By separating ingesters, each can be optimized for its data source while producing a unified output format.

## Full Architecture (Core + P2P Layer)
